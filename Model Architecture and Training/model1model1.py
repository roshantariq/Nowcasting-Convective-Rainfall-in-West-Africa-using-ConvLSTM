...
def create_shifted_frames(data):
    x = data[:, :3, :, :]  # First 3 frames as input
    y = data[:, 3:5, :, :]  # Next 2 frames as output
    return x, y
...
def build_model(input_shape):
    ...
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.2)(x)
    x = layers.ConvLSTM2D(32, (3,3), padding='same',return_sequences=True, activation='relu')(x)
    x = layers.BatchNormalization()(x)
...
try:
    with open('model1model1_lr.json', 'w', encoding='utf-8') as f:
...
try:
    with open('model1model1_lr.json', 'r', encoding='utf-8') as f:
