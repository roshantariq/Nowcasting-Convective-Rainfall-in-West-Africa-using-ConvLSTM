...
def create_shifted_frames(data):
    x = data[:, [0,3], :, :, :]  # 1,4 frames as input
    y = data[:, 4, :, :, :]  # 5 frame as output
    return x, y
...
try:
    with open('model4model4_lr.json', 'w', encoding='utf-8') as f:
...
try:
    with open('model4model4_lr.json', 'r', encoding='utf-8') as f:
